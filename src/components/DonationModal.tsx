          ...
          <div className="bg-slate-50 border-t border-slate-200 p-4 sm:p-5 flex items-center justify-between shrink-0">
            {step > 1 ? (
              <button
                ...
                className="px-4 py-2.5 rounded-xl border border-slate-300 text-slate-700 text-xs font-bold hover:bg-white flex items-center gap-1.5 cursor-pointer"
              >
              ...
              <div className="flex items-center gap-1.5 text-xs text-slate-500 font-medium">
                <Lock className="w-3.5 h-3.5 text-[#183D73]" />
                <span>Paiement chiffré 256-bit SSL</span>
              </div>
            )}
            ...
            <button
              ...
              className="px-6 py-3 rounded-xl bg-[#E8B84B] hover:bg-[#d4a338] text-[#0F2648] text-xs sm:text-sm font-extrabold uppercase tracking-wider flex items-center gap-2 cursor-pointer shadow-md transition-all transform hover:-translate-y-0.5"
            >
              {isProcessing ? (
                <>
                  <span className="w-4 h-4 rounded-full border-2 border-[#0F2648] border-t-transparent animate-spin" />
                  <span>Traitement sécurisé en cours...</span>
                </>
              ) : (
                <>
                  <span>
                    {step === 1 && "Continuer vers les Coordonnées"}
                    {step === 2 && "Procéder au Paiement"}
                    {step === 3 && `Confirmer le Don de $${amount} USD`}
                  </span>
